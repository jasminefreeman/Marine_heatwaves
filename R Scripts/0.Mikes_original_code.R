...
model<-e2e_read("North_Sea","1970-1999")    # SELECT YOUR REQUIRED MODEL

initial_state <- model$data$initial.state

for(kk in 1:N_YEAR_SETS){
  ...
  #########################################
  # HERE IS WHERE YOU WOULD INSERT YOUR CODE TO VARY THE INPUT DRIVING CONDITIONS ON A YEAR BY YEAR BASIS
  # EG INCREMENT FISHING GEAR ACTIVITY RATES EACH YEAR. AN EXAMPLE OF THIS IS PROVIDED FURTHER DOWN THIS SCRIPT
  #########################################
  ...
  if(kk>1){
    ...
    offshore_annual_group_land_disc$year<-kk
    inshore_annual_group_land_disc$year<-kk
    offshore_annual_group_gear_land_disc$year<-kk
    inshore_annual_group_gear_land_disc$year<-kk
...
e2e_plot_ts(model,cum_results,selection="ECO")
...
# EXAMPLE OF INCREMENTING FISHING ACTIVITY YEAR BY YEAR
#~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

# gear_mult is set to 1 in year 1 and ramps up lineally to 3 by year 5 and then stays steady for 5 years, and reverses
...
base_gear_mult<-model$data$fleet.model$gear_mult        # vector of gear activity rate multiplers
...
for(kk in 1:N_YEAR_SETS){
  ...
  new_gear_mult<-base_gear_mult
  
  #----------------------
  if(kk==2) new_gear_mult[]<-1.4
  if(kk==3) new_gear_mult[]<-1.8
  if(kk==4) new_gear_mult[]<-2.2
  if(kk==5) new_gear_mult[]<-2.6
  if(kk>5 & kk<11)  new_gear_mult[]<-3
  if(kk==11) new_gear_mult[]<-2.6
  if(kk==12) new_gear_mult[]<-2.2
  if(kk==13) new_gear_mult[]<-1.8
  if(kk==14) new_gear_mult[]<-1.4
  if(kk>14)  new_gear_mult[]<-1
  #----------------------
  
  newmodel$data$fleet.model$gear_mult<-new_gear_mult
...
# At the end, merge the cumulative results back onto a results object compativel with all other e2e functions
